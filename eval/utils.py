...
def prepare_dataset_for_eval(dataset_name, output_file):
    ...
    if dataset_name == '2wiki':
        with open('../2wiki/reslt.json', encoding='utf-8') as f:
            datas = json.load(f)
        question_string = "question"
    elif dataset_name == 'hotpotqa':
        with open('../hotpotqa/reslt.json',encoding='utf-8') as f:
            datas = json.load(f)
        question_string = 'question'
    else:
        print("dataset not found, you should pick from {2wiki,hotpotqa}.")
        exit(-1)
    with open(output_file, encoding='utf-8') as f:
        output_datas = json.load(f)
